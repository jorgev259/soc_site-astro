...
const DeletePublisher = z.object({ id: z.number() })
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    body = DeletePublisher.parse(formData)
    await prismaClient.publisher.findUniqueOrThrow({ where: { id: body.id }, select: { id: true } })
  } catch (err) {
    return Status(422, (err as Error).message)
  }

  try {
    await prismaClient.publisher.delete({ where: { id: body.id } })
