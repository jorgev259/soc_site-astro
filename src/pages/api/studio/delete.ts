...
const DeleteStudio = z.object({ slug: z.string() })
...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  try {
    ...
    body = DeleteStudio.parse(formData)
    await prismaClient.studio.findUniqueOrThrow({ where: { slug: body.slug }, select: { slug: true } })
  } catch (err) {
    return Status(422, (err as Error).message)
  }

  try {
    await prismaClient.studio.delete({ where: { slug: body.slug } })
